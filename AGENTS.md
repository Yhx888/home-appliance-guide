...
home-appliance-guide/
  ...
  backend/
    ...
    verification/           # web 多源核验结果（cat-1~17 全部批次已完成，见各品类 JSON）
...
置信度分布（verify 报告，实时聚合）：中 0.5~0.9 = 1704（84.9%）、低 0.3~0.5 = 219（10.9%）、极低 <0.3 = 84（4.2%）、高 ≥0.9 = 1、缺失 0。**核验管线已跑通 13 个品类（2026-08-07）**：108 组多源 verified，85 款产品带核验徽标；累计修正 15 处参数（含集成灶热效率火力错位 20 处清理）；剩余品类少量型号待后续批次。前端按产品展示核验徽标（✓已核验 / 部分核验）。
